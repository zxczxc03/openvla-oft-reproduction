...
def make_dataset_from_rlds(
    ...
    *,
    ...
    Args:
        ...
        include_value_targets (bool): Preserve trajectory-level `reward` and `return_to_go` generated by the
            standardization transform. Intended for value-function training only.
        include_metadata (bool): Preserve a stable numeric trajectory id as `episode_id` and semantic task metadata.
            The id is copied from dlimp's `_traj_index`; raw file shuffling and parallel file interleaving are disabled
            so the same dataset/split maps each id to the same trajectory across runs.
    ...
    Returns:
        ...
        - episode_id                    # stable trajectory id within the dataset/split, if `include_metadata=True`
    """
    ...
    def restructure(traj):
        ...
        # add timestep info
        new_obs["timestep"] = tf.cast(traj.get("_frame_index", tf.range(traj_len)), tf.int32)
        ...
        if include_value_targets:
            missing_keys = {"reward", "return_to_go"} - set(traj)
            if missing_keys:
                raise ValueError(
                    f"Value-function dataset `{name}` is missing target keys: {missing_keys}. "
                    "Its standardization transform must create reward and return_to_go."
                )
            value_targets = {
                "reward": tf.cast(traj["reward"], tf.float32),
                "return_to_go": tf.cast(traj["return_to_go"], tf.float32),
            }
        ...
        if include_metadata:
            if "_traj_index" not in traj:
                raise ValueError(
                    f"Dataset `{name}` is missing dlimp `_traj_index`; cannot create stable episode_id."
                )
            metadata_targets["episode_id"] = tf.cast(traj["_traj_index"], tf.int64)
    ...
    read_shuffle = False if include_metadata else shuffle
    read_parallelism = 1 if include_metadata else num_parallel_reads
    dataset = dl.DLataset.from_rlds(builder, split=split, shuffle=read_shuffle, num_parallel_reads=read_parallelism)
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    # 如果一条 trajectory 太长，就从里面随机抽取 subsample_length 个时间步/window，丢掉其余部分。
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        # 原来数据集一个元素为一条trajectory，flatten后一个元素为一个Step
