...
class FinetuneConfig:
    ...
    quantile_data_path: Path = Path("advantage_data") / "quantile_by_semantic_train.npy"
    recap_negative_loss_weight: float = 0.0           # Weight for samples labeled `Advantage: negative`
...
def load_recap_advantage_lookup(table_path: Path) -> Dict[Tuple[int, int], Tuple[float, Optional[str]]]:
    with np.load(table_path, allow_pickle=False) as data:
        required = {"episode_id", "timestep", "advantage"}
        missing = required - set(data.files)
        if missing:
            raise KeyError(f"RECAP advantage table `{table_path}` is missing keys: {sorted(missing)}")

        semantic_tasks = data["semantic_task"] if "semantic_task" in data.files else None
        lookup = {}
        for idx, (episode_id, timestep, advantage) in enumerate(
            zip(data["episode_id"], data["timestep"], data["advantage"])
        ):
            semantic_task = decode_scalar_string(semantic_tasks[idx]) if semantic_tasks is not None else None
            lookup[(int(episode_id), int(timestep))] = (float(advantage), semantic_task)
    return lookup


def build_recap_semantic_task_map(quantile_data: Dict[str, float]) -> Dict[str, str]:
    semantic_task_map = {}
    for semantic_task in quantile_data:
        language = " ".join(str(semantic_task).replace("_", " ").lower().split())
        semantic_task_map[language] = semantic_task
        semantic_task_map[normalize_language(language)] = semantic_task

    for language, base_tasks in NORMALIZED_LANGUAGE_TO_BASE_TASKS.items():
        if not base_tasks:
            continue
        semantic_task = base_tasks[0].replace(" ", "_")
        if semantic_task in quantile_data:
            semantic_task_map[normalize_language(language)] = semantic_task
    return semantic_task_map
...
def run_forward_pass(
    ...
    recap_loss_weights = batch.get("recap_loss_weights")
    if recap_loss_weights is not None:
        recap_loss_weights = recap_loss_weights.to(device_id).to(torch.float32)
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if use_l1_regression:
            ...
            # Get full L1 loss
            if recap_loss_weights is None:
                loss = torch.nn.L1Loss()(ground_truth_actions, predicted_actions)
            else:
                per_sample_l1_loss = torch.abs(ground_truth_actions - predicted_actions).float().mean(dim=(1, 2))
                loss = (per_sample_l1_loss * recap_loss_weights).sum() / recap_loss_weights.sum().clamp_min(1.0)
        ...
        if use_diffusion:
            ...
            if recap_loss_weights is None:
                ...
                per_sample_mse_loss = nn.functional.mse_loss(noise_pred, noise, reduction="none")
                per_sample_mse_loss = per_sample_mse_loss.float().mean(dim=(1, 2))
                loss = (per_sample_mse_loss * recap_loss_weights).sum() / recap_loss_weights.sum().clamp_min(1.0)
        ...
        if recap_loss_weights is not None:
            metrics["recap_loss_weight_mean"] = recap_loss_weights.mean().item()
            metrics["recap_loss_weight_positive_frac"] = (recap_loss_weights > 0).float().mean().item()
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # load advantage data and quantile data

    mix = OXE_NAMED_MIXTURES[cfg.dataset_name]
    dataset_names, _ = zip(*mix)
    positive_datasets = set(cfg.recap_positive_datasets)
    advantage_data = {}
    for dataset_name in dataset_names:
        if dataset_name in positive_datasets:
            continue
        table_path = cfg.advantage_data_dir / dataset_name / "train" / f"advantage_table_{dataset_name}.npz"
        advantage_data[dataset_name] = load_recap_advantage_lookup(table_path)
    quantile_data = np.load(cfg.quantile_data_path, allow_pickle=True).item()
    semantic_task_map = build_recap_semantic_task_map(quantile_data)
    ...
    batch_transform = RLDSBatchTransform(
        ...
        recap_positive_datasets=cfg.recap_positive_datasets,
        recap_semantic_task_map=semantic_task_map,
        recap_negative_loss_weight=cfg.recap_negative_loss_weight,
    )
    ...
    recent_metrics = {
        ...
        "recap_loss_weight_mean": deque(maxlen=cfg.grad_accumulation_steps),
        "recap_loss_weight_positive_frac": deque(maxlen=cfg.grad_accumulation_steps),
    }
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if did_optimizer_step and gradient_step_idx > 0 and log_step % cfg.save_freq == 0:
